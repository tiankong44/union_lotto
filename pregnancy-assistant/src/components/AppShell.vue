...
const navItems = [
  ...
  { to: '/movement', label: '记胎动', icon: Activity },
  { to: '/records', label: '记录', icon: CalendarDays },
...
<template>
  <div class="app-frame">
    <aside class="side-rail">
      <RouterLink class="brand-lockup" to="/" aria-label="回到总览">
    ...
    <main class="main-column">
      ...
      <nav class="bottom-nav" aria-label="移动端主要导航">
        <RouterLink v-for="item in navItems.slice(0, 5)" :key="item.to" :to="item.to" class="bottom-nav-item">
